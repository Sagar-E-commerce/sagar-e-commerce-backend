import { Controller, UploadedFile, UseInterceptors, Post,Get,Patch,Delete, BadRequestException, Query, InternalServerErrorException, Body, Param, UseGuards, Req } from "@nestjs/common";
...
export class AdminMgtController{
    ...
    async SearchStaff(@Query('keyword')keyword:string|any){
        return await this.adminsmgtservice.SearchForOtherAdmin(keyword)
    }

    @AdminAccessLevel(AdminAccessLevels.LEVEL3)
    @Patch('/change-other-admin-accesslevel/:adminID')
    async ChangeOtherAdminAccessLevel(@Param('adminID')adminID:number, @Body()dto:AdminchangeOtherAdminAccessLevelDto){
        return await this.adminsmgtservice.ChangeOtherAdminAccessLevel(adminID,dto)

    }

    @AdminAccessLevel(AdminAccessLevels.LEVEL3)
    @Patch('/change-other-admin-admintype/:adminID')
    async ChangesOtherAdminAdmintype(@Param('adminID')adminID:number, @Body()dto:AdminChangeOtherAdminTypeDto){
